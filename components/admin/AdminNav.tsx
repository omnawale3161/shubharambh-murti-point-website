...
export function AdminNav() {
  return (
    <aside className="border-b border-outline-variant bg-white p-4 md:min-h-screen md:border-b-0 md:border-r">
      <Link href="/admin" className="font-serif text-xl font-bold text-primary">Store Admin</Link>
      <nav className="mt-5 flex gap-2 overflow-auto md:grid" aria-label="Admin navigation">
      ...
      <form action={adminLogoutAction} className="mt-5">
        <button className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-bold text-primary hover:bg-surface-container"><LogOut size={17} />Sign out</button>
      </form>
    </aside>
  );
}
